...
public class CountriesFetcher {
    ...
    public static CountryList getCountries(Context context) {
        ...
        try {
            JSONArray countries = new JSONArray(getJsonFromRaw(context));
            for (int i = 0; i < countries.length(); i++) {
                try {
                    JSONObject country = (JSONObject) countries.get(i);
                    int countryResIdentifier= context.getApplicationContext().getResources().getIdentifier(country.getString("name"),
                            "string", context.getPackageName());
                    System.out.println(country.getString("name"));
                    String s = context.getResources().getString(countryResIdentifier);
                    mCountries.add(new Country(s, country.getString("iso2"), country.getInt("dialCode")));
    ...
    public static class CountryList extends ArrayList<Country> {
        ...
        public int indexOfName(String name) {
            for (int i = 0; i < this.size(); i++) {
                if (this.get(i).getName().toUpperCase().equals(name.toUpperCase())) {
                    return i;
                }
            }
            return -1;
