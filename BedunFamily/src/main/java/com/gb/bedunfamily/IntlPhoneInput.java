...
public class IntlPhoneInput extends RelativeLayout implements View.OnClickListener {
    ...
    // UI Views
    private ImageView mCountrySpinner;
    private EditText mPhoneEdit;

    //Adapters
    private PhoneCountrySpinnerAdapter mCountrySpinnerAdapter;
    ...
    private CustomCountryDialog cdd;
    ...
    private void init(AttributeSet attrs) {
        ...
        mCountrySpinner.setOnClickListener(this);
     ...
     */
    private void setEmptyDefault(String iso) {
        try {
            if (iso == null || iso.isEmpty()) {
                iso = DEFAULT_COUNTRY;
            }
            int defaultIdx = mCountries.indexOfIso(iso);
            System.out.println(defaultIdx);
            mSelectedCountry = mCountries.get(defaultIdx);
            mCountrySpinner.setImageResource(getContext().getResources().getIdentifier("country_"
                    + mSelectedCountry.getIso().toLowerCase(), "drawable", getContext().getPackageName()));
    ...
    //getApplicationContext
        cdd = new CustomCountryDialog((Activity) getContext());
        ...
        titleView.setText(R.string.country_area_code);
        listView = view.findViewById(R.id.list_dialog);
        mCountrySpinnerAdapter = new PhoneCountrySpinnerAdapter(getContext(), countriesNameArray);
        mCountrySpinnerAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        listView.setAdapter(mCountrySpinnerAdapter);
        ...
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view,
                ...
                int index = mCountries.indexOfName(name);
                mSelectedCountry = mCountries.get(index);
                mCountrySpinner.setImageResource(getContext().getResources().getIdentifier("country_"
                        + mSelectedCountry.getIso().toLowerCase(), "drawable", getContext().getPackageName()));
                //Make sure that the watcher is added into the listeners of the edittext
                //after updating the country selected...
        ...
        searchView.setOnQueryTextListener(new android.support.v7.widget.SearchView.OnQueryTextListener(){
            ...
            public boolean onQueryTextChange(String newText) {
                mCountrySpinnerAdapter.getFilter().filter(newText);
                return false;
            }
        });
    }
    ...
    private class PhoneNumberWatcher extends PhoneNumberFormattingTextWatcher {
        ...
        public void onTextChanged(CharSequence s, int start, int before, int count) {
            ...
            Activity loginActivity = (Activity) mPhoneEdit.getContext();
            TextView tv = loginActivity.findViewById(R.id.textViewUserIdMessage);
            tv.setTextColor(Color.WHITE);
            tv.setText("");
            TextView label = loginActivity.findViewById(R.id.textViewUserIdLabel);
    ...
    public void setNumber(String number) {
        try {
            ...
            listView.setSelection(countryIdx);
